...
EC_EXPORT MStatus initializePlugin(MObject obj)
{
  ...
  if (deleteAllArchivesCallbackOnNewId == 0)
    deleteAllArchivesCallbackOnNewId = MSceneMessage::addCallback(
        MSceneMessage::kBeforeNew, deleteAllArchivesCallback);
  if (deleteAllArchivesCallbackOnOpenId == 0)
    deleteAllArchivesCallbackOnOpenId = MSceneMessage::addCallback(
        MSceneMessage::kBeforeOpen, deleteAllArchivesCallback);
  if (deleteAllArchivesCallbackOnExitId == 0)
    deleteAllArchivesCallbackOnExitId = MSceneMessage::addCallback(
        MSceneMessage::kMayaExiting, deleteAllArchivesCallback);
  ...
  status = plugin.registerCommand("ExocortexAlembic_meshToSubdiv",
                                  AlembicPolyMeshToSubdivCommand::creator,
                                  AlembicPolyMeshToSubdivCommand::createSyntax);

  // nodes
  status = plugin.registerNode(
      "ExocortexAlembicTimeControl", mTimeControlNodeId,
      &AlembicTimeControlNode::creator, &AlembicTimeControlNode::initialize);
  status = plugin.registerNode("ExocortexAlembicFile", mFileNodeId,
                               &AlembicFileNode::creator,
                               &AlembicFileNode::initialize);
  status = plugin.registerNode("ExocortexAlembicXform", mXformNodeId,
                               &AlembicXformNode::creator,
                               &AlembicXformNode::initialize);
  status = plugin.registerNode("ExocortexAlembicCamera", mCameraNodeId,
                               &AlembicCameraNode::creator,
                               &AlembicCameraNode::initialize);
  status = plugin.registerNode("ExocortexAlembicPolyMesh", mPolyMeshNodeId,
                               &AlembicPolyMeshNode::creator,
                               &AlembicPolyMeshNode::initialize);
  status = plugin.registerNode("ExocortexAlembicSubD", mSubDNodeId,
                               &AlembicSubDNode::creator,
                               &AlembicSubDNode::initialize);
  status = plugin.registerNode(
      "ExocortexAlembicPolyMeshDeform", mPolyMeshDeformNodeId,
      &AlembicPolyMeshDeformNode::creator,
      &AlembicPolyMeshDeformNode::initialize, MPxNode::kDeformerNode);
  status = plugin.registerNode("ExocortexAlembicSubDDeform", mSubDDeformNodeId,
                               &AlembicSubDDeformNode::creator,
                               &AlembicSubDDeformNode::initialize,
                               MPxNode::kDeformerNode);
  status = plugin.registerNode(
      "ExocortexAlembicPoints", mPointsNodeId, &AlembicPointsNode::creator,
      &AlembicPointsNode::initialize, MPxNode::kEmitterNode);
  status = plugin.registerNode("ExocortexAlembicCurves", mCurvesNodeId,
                               &AlembicCurvesNode::creator,
                               &AlembicCurvesNode::initialize);
  status = plugin.registerNode(
      "ExocortexAlembicCurvesDeform", mCurvesDeformNodeId,
      &AlembicCurvesDeformNode::creator, &AlembicCurvesDeformNode::initialize,
      MPxNode::kDeformerNode);
  status = plugin.registerNode(
      "ExocortexAlembicCurvesLocator", mCurvesLocatorNodeId,
      &AlembicCurvesLocatorNode::creator, &AlembicCurvesLocatorNode::initialize,
      MPxNode::kLocatorNode);

  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicXform", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicCamera", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicPolyMesh", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicSubD", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicPolyMeshDeform", NULL,
                                     &status);
  MDGMessage::addNodeRemovedCallback(
      removeExocortexAlembicNode, "ExocortexAlembicSubDDeform", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicPoints", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicCurves", NULL, &status);
  MDGMessage::addNodeRemovedCallback(removeExocortexAlembicNode,
                                     "ExocortexAlembicCurvesDeform", NULL,
                                     &status);

  // Load the menu!
  MString cmd = "source \"menu.mel\"; exocortexAlembicLoadMenu(\"" +
                plugin.name() + "\");";
  MStatus commandStatus = MGlobal::executeCommand(cmd, true, false);
  if (commandStatus != MStatus::kSuccess) {
    // EC_LOG_ERROR("FAILED TO SOURCE ../scripts/menu.mel: " <<
    // commandStatus.errorString());
  }
...
EC_EXPORT MStatus uninitializePlugin(MObject obj)
{
  ...
  // Unload the menu!
  MStatus commandStatus = MGlobal::executeCommand(
      "source \"menu.mel\"; exocortexAlembicUnloadMenu;", true, false);
  if (commandStatus != MStatus::kSuccess) {
    // EC_LOG_ERROR("FAILED TO SOURCE ../scripts/menu.mel: " <<
    // commandStatus.errorString());
  }
